/*
 * shrink_block.c - LZSA2 block compressor implementation
 *
 ...
 */
static int lzsa_write_nibble(unsigned char *pOutData, int nOutOffset, const int nMaxOutDataSize, int *nCurNibbleOffset, int nNibbleValue) {
   if (nOutOffset < 0) return -1;

   if ((*nCurNibbleOffset) == -1) {
      if (nOutOffset >= nMaxOutDataSize) return -1;
      (*nCurNibbleOffset) = nOutOffset;
      pOutData[nOutOffset++] = (nNibbleValue ^ 0x0f) << 4;
   }
   else {
...
static inline int lzsa_get_literals_varlen_size(const int nLength) {
   if (nLength < LITERALS_RUN_LEN) {
      ...
      if (nLength < (LITERALS_RUN_LEN + 15)) {
         return 4;
      }
      else {
         if (nLength < LITERALS_RUN_LEN + 15 + 255)
            return 4+8;
         else {
            return 4+24;
         }
 ...
 * @param nLength literals length
 */
static inline int lzsa_write_literals_varlen(unsigned char *pOutData, int nOutOffset, const int nMaxOutDataSize, int *nCurNibbleOffset, int nLength) {
   if (nLength >= LITERALS_RUN_LEN) {
      if (nLength < (LITERALS_RUN_LEN + 15)) {
         nOutOffset = lzsa_write_nibble(pOutData, nOutOffset, nMaxOutDataSize, nCurNibbleOffset, nLength - LITERALS_RUN_LEN + 1);
      }
      else {
         ...
         if (nLength < LITERALS_RUN_LEN + 15 + 255)
            pOutData[nOutOffset++] = nLength - (LITERALS_RUN_LEN + 15) + 1;
         else {
 ...
 * @param nLength encoded match length (actual match length - MIN_MATCH_SIZE)
 */
static inline int lzsa_write_match_varlen(unsigned char *pOutData, int nOutOffset, const int nMaxOutDataSize, int *nCurNibbleOffset, int nLength) {
   if (nLength >= MATCH_RUN_LEN) {
      if (nLength < (MATCH_RUN_LEN + 15)) {
         nOutOffset = lzsa_write_nibble(pOutData, nOutOffset, nMaxOutDataSize, nCurNibbleOffset, nLength - MATCH_RUN_LEN + 1);
      }
      else {
         nOutOffset = lzsa_write_nibble(pOutData, nOutOffset, nMaxOutDataSize, nCurNibbleOffset, 0);
         if (nOutOffset < 0) return -1;
 ...
 */
static void lzsa_insert_forward_match(lzsa_compressor *pCompressor, const unsigned char *pInWindow, const int i, const int nMatchOffset, const int nStartOffset, const int nEndOffset, int nDepth) {
   ...
   for (j = 0; j < NARRIVALS_PER_POSITION_BIG && arrival[j].from_slot; j++) {
      int nRepOffset = arrival[j].rep_offset;

      if (nMatchOffset != nRepOffset && nRepOffset && arrival[j].rep_len >= MIN_MATCH_SIZE) {
            ...
            if (visited[nRepPos].offset != nMatchOffset || visited[nRepPos].length > nRepLen) {
               ...
               if (pInWindow[nRepPos] == pInWindow[nRepPos - nMatchOffset]) {
                  ...
                  if (nMinLen >= nRepLen || !memcmp(pInWindow + nRepPos + nMinLen, pInWindow + nRepPos + nMinLen - nMatchOffset, nRepLen - nMinLen)) {
                     ...
                     lzsa_match* fwd_match = pCompressor->match + ((nRepPos - nStartOffset) << MATCHES_PER_INDEX_SHIFT);
                     int r;
                     ...
                     if (r < NMATCHES_PER_INDEX) {
                        ...
                        if (nDepth < 9)
                           lzsa_insert_forward_match(pCompressor, pInWindow, nRepPos, nMatchOffset, nStartOffset, nEndOffset, nDepth + 1);
                     }
                  }
               }
            }
 ...
 */
static void lzsa_optimize_forward(lzsa_compressor *pCompressor, const unsigned char *pInWindow, lzsa_match *pBestMatch, const int nStartOffset, const int nEndOffset, const int nReduce, const int nInsertForwardReps, const int nArrivalsPerPosition) {
   ...
   const int nLeaveAloneMatchSize = (nArrivalsPerPosition == NARRIVALS_PER_POSITION_SMALL) ? LEAVE_ALONE_MATCH_SIZE_SMALL : LEAVE_ALONE_MATCH_SIZE;
   int i, j, n;
   ...
   for (i = nStartOffset; i != nEndOffset; i++) {
      ...
      for (j = 0; j < nArrivalsPerPosition && cur_arrival[j].from_slot; j++) {
         ...
         if (nNumLiterals == LITERALS_RUN_LEN) {
            nCodingChoiceCost += 4;
         }
         else if (nNumLiterals == (LITERALS_RUN_LEN + 15)) {
            nCodingChoiceCost += 8;
         }
      ...
      lzsa_match *match = pCompressor->match + ((i - nStartOffset) << MATCHES_PER_INDEX_SHIFT);
      int nNumArrivalsForThisPos = j, nMinOverallRepLen = 0, nMaxOverallRepLen = 0;

      int nRepMatchArrivalIdxAndLen[(NARRIVALS_PER_POSITION_BIG * 2) + 1];
      ...
      for (j = 0; j < nNumArrivalsForThisPos && (i + MIN_MATCH_SIZE) <= nEndOffset; j++) {
         int nRepOffset = cur_arrival[j].rep_offset;
      ...
      for (m = 0; m < NMATCHES_PER_INDEX && match[m].length; m++) {
         ...
         if (nInsertForwardReps)
            lzsa_insert_forward_match(pCompressor, pInWindow, i, nMatchOffset, nStartOffset, nEndOffset, 0);
         ...
         for (k = nStartingMatchLen; k <= nMatchLen; k++) {
            if (k == (MATCH_RUN_LEN + MIN_MATCH_SIZE)) {
               nMatchLenCost = 4 + 8 /* token */;
            }
            else {
               if (k == (MATCH_RUN_LEN + 15 + MIN_MATCH_SIZE))
                     ...
                     if (!exists) {
                        if (n < nArrivalsPerPosition - 1) {
                           ...
                           if (!exists) {
                              ...
                              if (z == (nArrivalsPerPosition - 1) && pDestSlots[z].from_slot && pDestSlots[z].match_len < MIN_MATCH_SIZE)
                                 z--;
         ...
         if (nMatchLen >= LCP_MAX && ((m + 1) >= NMATCHES_PER_INDEX || match[m + 1].length < LCP_MAX))
            break;
      }
   }
 ...
 */
static int lzsa_optimize_command_count(lzsa_compressor *pCompressor, const unsigned char *pInWindow, lzsa_match *pBestMatch, const int nStartOffset, const int nEndOffset) {
   int i;
   int nNumLiterals = 0;
   ...
   for (i = nStartOffset; i < nEndOffset; ) {
      ...
      if (pMatch->length == 0 &&
         (i + 1) < nEndOffset &&
         pBestMatch[i + 1].length >= MIN_MATCH_SIZE &&
         pBestMatch[i + 1].length < MAX_VARLEN &&
         pBestMatch[i + 1].offset &&
         i >= pBestMatch[i + 1].offset &&
         (i + pBestMatch[i + 1].length + 1) <= nEndOffset &&
         !memcmp(pInWindow + i - (pBestMatch[i + 1].offset), pInWindow + i, pBestMatch[i + 1].length + 1)) {
         int nCurLenSize = lzsa_get_match_varlen_size(pBestMatch[i + 1].length - MIN_MATCH_SIZE);
         int nReducedLenSize = lzsa_get_match_varlen_size(pBestMatch[i + 1].length + 1 - MIN_MATCH_SIZE);
      ...
      if (pMatch->length >= MIN_MATCH_SIZE) {
         if ((i + pMatch->length) < nEndOffset /* Don't consider the last match in the block, we can only reduce a match inbetween other tokens */) {
            ...
            if (nNextIndex < nEndOffset && pBestMatch[nNextIndex].length >= MIN_MATCH_SIZE) {
               ...
               if (pBestMatch[nNextIndex].offset && pMatch->offset != pBestMatch[nNextIndex].offset && nRepMatchOffset != pBestMatch[nNextIndex].offset) {
                  ...
                  if (i > pBestMatch[nNextIndex].offset && (i - pBestMatch[nNextIndex].offset + pMatch->length) <= nEndOffset) {
                     ...
                     if (nMaxLen >= pMatch->length) {
                        ...
                        nPartialSizeBefore = lzsa_get_match_varlen_size(pMatch->length - MIN_MATCH_SIZE);
                        nPartialSizeBefore += (pMatch->offset <= 32) ? 4 : ((pMatch->offset <= 512) ? 8 : ((pMatch->offset <= (8192 + 512)) ? 12 : 16));
                        nPartialSizeBefore += lzsa_get_literals_varlen_size(nNextLiterals);

                        nPartialSizeAfter = lzsa_get_match_varlen_size(nMaxLen - MIN_MATCH_SIZE);
                        nPartialSizeAfter += lzsa_get_literals_varlen_size(nNextLiterals + (pMatch->length - nMaxLen)) + ((pMatch->length - nMaxLen) << 3);
               ...
               if (pMatch->length < 9 /* Don't waste time considering large matches, they will always win over literals */) {
                  ...
                  int nCurCommandSize = 8 /* token */ + lzsa_get_literals_varlen_size(nNumLiterals) + lzsa_get_match_varlen_size(pMatch->length - MIN_MATCH_SIZE);
                  ...
                  int nNextCommandSize = 8 /* token */ + lzsa_get_literals_varlen_size(nNextLiterals) + /* (nNextLiterals << 3) + */ lzsa_get_match_varlen_size(pBestMatch[nNextIndex].length - MIN_MATCH_SIZE);
                  ...
                  int nReducedCommandSize = (pMatch->length << 3) + 8 /* token */ + lzsa_get_literals_varlen_size(nNumLiterals + pMatch->length + nNextLiterals) + /* (nNextLiterals << 3) + */ lzsa_get_match_varlen_size(pBestMatch[nNextIndex].length - MIN_MATCH_SIZE);
                  ...
                  if (nRepMatchOffset && nRepMatchOffset != nPrevRepMatchOffset && nRepMatchLen >= MIN_MATCH_SIZE && nRepMatchOffset != pBestMatch[nNextIndex].offset && nRepIndex > pBestMatch[nNextIndex].offset &&
         ...
         if ((i + pMatch->length) <= nEndOffset && pMatch->offset > 0 && pMatch->length >= MIN_MATCH_SIZE &&
            pBestMatch[i + pMatch->length].offset > 0 &&
            pBestMatch[i + pMatch->length].length >= MIN_MATCH_SIZE &&
            ...
            int nCurPartialSize = lzsa_get_match_varlen_size(pMatch->length - MIN_MATCH_SIZE);

            nCurPartialSize += 8 /* token */ + /* lzsa_get_literals_varlen_size(0) + */ lzsa_get_match_varlen_size(pBestMatch[i + pMatch->length].length - MIN_MATCH_SIZE);
            ...
            int nReducedPartialSize = lzsa_get_match_varlen_size(pMatch->length + pBestMatch[i + pMatch->length].length - MIN_MATCH_SIZE);
 ...
 */
static int lzsa_get_compressed_size(lzsa_compressor *pCompressor, lzsa_match *pBestMatch, const int nStartOffset, const int nEndOffset) {
   int i;
   int nNumLiterals = 0;
   int nRepMatchOffset = 0;
   ...
   for (i = nStartOffset; i < nEndOffset; ) {
      ...
      if (pMatch->length >= MIN_MATCH_SIZE) {
         ...
         int nCommandSize = 8 /* token */ + lzsa_get_literals_varlen_size(nNumLiterals) + (nNumLiterals << 3) + nOffsetSize /* match offset */ + lzsa_get_match_varlen_size(nEncodedMatchLen);
 ...
 */
static int lzsa_write_block(lzsa_compressor *pCompressor, lzsa_match *pBestMatch, const unsigned char *pInWindow, const int nStartOffset, const int nEndOffset, unsigned char *pOutData, const int nMaxOutDataSize) {
   int i;
   int nNumLiterals = 0;
   ...
   for (i = nStartOffset; i < nEndOffset; ) {
      ...
      if (pMatch->length >= MIN_MATCH_SIZE) {
         ...
         if (nMatchOffset == nRepMatchOffset) {
            nTokenOffsetMode = 0x20;
            nOffsetSize = 0;
         }
         else {
            if (nMatchOffset <= 32) {
               nTokenOffsetMode = 0xc0 | (((nMatchOffset - 1) & 1) << 5);
               nOffsetSize = 4;
            }
            else if (nMatchOffset <= 512) {
               nTokenOffsetMode = 0x40 | (((nMatchOffset - 1) & 1) << 5);
               nOffsetSize = 8;
            }
            else if (nMatchOffset <= (8192 + 512)) {
               nTokenOffsetMode = 0x80 | (((nMatchOffset - 1 - 512) & 0x0100) >> 3);
               nOffsetSize = 12;
            }
            else {
               nTokenOffsetMode = 0x00;
               nOffsetSize = 16;
            }
         }
         ...
         pOutData[nOutOffset++] = nTokenOffsetMode | (nTokenMatchLen << 2) | nTokenLiteralsLen;
         nOutOffset = lzsa_write_literals_varlen(pOutData, nOutOffset, nMaxOutDataSize, &nCurNibbleOffset, nNumLiterals);
         if (nOutOffset < 0) return -1;
         ...
         if (nTokenOffsetMode == 0xc0 || nTokenOffsetMode == 0xe0) {
            nOutOffset = lzsa_write_nibble(pOutData, nOutOffset, nMaxOutDataSize, &nCurNibbleOffset, (nMatchOffset - 1) >> 1);
            if (nOutOffset < 0) return -1;
         }
         else if (nTokenOffsetMode == 0x40 || nTokenOffsetMode == 0x60) {
            pOutData[nOutOffset++] = (nMatchOffset - 1) >> 1;
         }
         else if (nTokenOffsetMode == 0x80 || nTokenOffsetMode == 0xa0) {
            nOutOffset = lzsa_write_nibble(pOutData, nOutOffset, nMaxOutDataSize, &nCurNibbleOffset, ((nMatchOffset - 1 - 512) >> 9) & 0x0f);
            if (nOutOffset < 0) return -1;
            pOutData[nOutOffset++] = (nMatchOffset - 1 - 512) & 0xff;
         }
         else if (nTokenOffsetMode == 0x00) {
         ...
         nOutOffset = lzsa_write_match_varlen(pOutData, nOutOffset, nMaxOutDataSize, &nCurNibbleOffset, nEncodedMatchLen);
         if (nOutOffset < 0) return -1;
...

   {
      ...
      pOutData[nOutOffset++] = nTokenLiteralsLen | 0x3c;
      nOutOffset = lzsa_write_literals_varlen(pOutData, nOutOffset, nMaxOutDataSize, &nCurNibbleOffset, nNumLiterals);
      if (nOutOffset < 0) return -1;
   ...
   nOutOffset = lzsa_write_nibble(pOutData, nOutOffset, nMaxOutDataSize, &nCurNibbleOffset, 0);   /* Extended match length nibble */
   if (nOutOffset < 0) return -1;
 ...
 * @return size of compressed data in output buffer, or -1 if the data is uncompressible
 */
static int lzsa_write_raw_uncompressed_block(lzsa_compressor *pCompressor, const unsigned char *pInWindow, const int nStartOffset, const int nEndOffset, unsigned char *pOutData, const int nMaxOutDataSize) {
   int nCurNibbleOffset = -1;
   int nNumLiterals = nEndOffset - nStartOffset;
   int nTokenLiteralsLen = (nNumLiterals >= LITERALS_RUN_LEN) ? LITERALS_RUN_LEN : nNumLiterals;
   int nOutOffset = 0;

   int nCommandSize = 8 /* token */ + lzsa_get_literals_varlen_size(nNumLiterals) + (nNumLiterals << 3) + 4 + 8;
   if ((nOutOffset + ((nCommandSize + 7) >> 3)) > nMaxOutDataSize)
      return -1;
 ...
 * @return size of compressed data in output buffer, or -1 if the data is uncompressible
 */
int lzsa_optimize_and_write_block(lzsa_compressor *pCompressor, const unsigned char *pInWindow, const int nPreviousBlockSize, const int nInDataSize, unsigned char *pOutData, const int nMaxOutDataSize) {
   int nResult, nBaseCompressedSize;
   int nArrivalsPerPosition = (nInDataSize < 65536) ? NARRIVALS_PER_POSITION_BIG : NARRIVALS_PER_POSITION_SMALL;
   ...
   lzsa_optimize_forward(pCompressor, pInWindow, pCompressor->best_match - nPreviousBlockSize, nPreviousBlockSize, nPreviousBlockSize + nInDataSize, 0 /* reduce */, (nInDataSize < 65536) ? 1 : 0 /* insert forward reps */, nArrivalsPerPosition);

   int nDidReduce;
   int nPasses = 0;
   do {
      nDidReduce = lzsa_optimize_command_count(pCompressor, pInWindow, pCompressor->best_match - nPreviousBlockSize, nPreviousBlockSize, nPreviousBlockSize + nInDataSize);
      nPasses++;
   } while (nDidReduce && nPasses < 20);

   nBaseCompressedSize = lzsa_get_compressed_size(pCompressor, pCompressor->best_match - nPreviousBlockSize, nPreviousBlockSize, nPreviousBlockSize + nInDataSize);
   ...
   if (nBaseCompressedSize > 0 && nInDataSize < 65536) {
      ...
      lzsa_optimize_forward(pCompressor, pInWindow, pCompressor->improved_match - nPreviousBlockSize, nPreviousBlockSize, nPreviousBlockSize + nInDataSize, 1 /* reduce */, 0 /* use forward reps */, nArrivalsPerPosition);

      nPasses = 0;
      do {
         nDidReduce = lzsa_optimize_command_count(pCompressor, pInWindow, pCompressor->improved_match - nPreviousBlockSize, nPreviousBlockSize, nPreviousBlockSize + nInDataSize);
         nPasses++;
      } while (nDidReduce && nPasses < 20);

      nReducedCompressedSize = lzsa_get_compressed_size(pCompressor, pCompressor->improved_match - nPreviousBlockSize, nPreviousBlockSize, nPreviousBlockSize + nInDataSize);
      if (nReducedCompressedSize > 0 && nReducedCompressedSize <= nBaseCompressedSize) {
         ...
         for (nPosition = nPreviousBlockSize + 1; nPosition < (nEndOffset - 1); nPosition++) {
            lzsa_match* match = pCompressor->match + ((nPosition - nPreviousBlockSize) << MATCHES_PER_INDEX_SHIFT);
            int m = 0, nInserted = 0;
            int nMatchPos;
                  ...
                  if (nMatchOffset <= MAX_OFFSET) {
                     ...
                     if (!nAlreadyExists) {
                        ...
                        if (nGotMatch) {
                           ...
                           lzsa_insert_forward_match(pCompressor, pInWindow, nPosition, nMatchOffset, nPreviousBlockSize, nEndOffset, 8);
                           break;
                        }
                     }
                  }
         ...
         memset(pCompressor->best_match, 0, BLOCK_SIZE * sizeof(lzsa_match));
         lzsa_optimize_forward(pCompressor, pInWindow, pCompressor->best_match - nPreviousBlockSize, nPreviousBlockSize, nPreviousBlockSize + nInDataSize, 1 /* reduce */, 0 /* use forward reps */, nArrivalsPerPosition);
         ...
         nSupplementedCompressedSize = lzsa_get_compressed_size(pCompressor, pCompressor->best_match - nPreviousBlockSize, nPreviousBlockSize, nPreviousBlockSize + nInDataSize);
   ...
   nResult = lzsa_write_block(pCompressor, pBestMatch, pInWindow, nPreviousBlockSize, nPreviousBlockSize + nInDataSize, pOutData, nMaxOutDataSize);
   if (nResult < 0) {
      nResult = lzsa_write_raw_uncompressed_block(pCompressor, pInWindow, nPreviousBlockSize, nPreviousBlockSize + nInDataSize, pOutData, nMaxOutDataSize);
   }
